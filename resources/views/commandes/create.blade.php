                ...
                <div class="col-md-10">
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between align-items-center">
                                <h2 class="m-0 font-weight-bold">
                                    <i class="mdi mdi-cart-plus"></i> Créer une Nouvelle Commande
                                </h2>
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{ route('commandes.store') }}" id="commande-form">
                                @csrf
                                <div class="row mb-4">
                                    <div class="col-md-8">
                                        <!-- Article Search Section -->
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="form-group mb-3">
                                                    ...
                                                    <textarea class="form-control" rows="3" id="notes" name="notes" rows="3"
                                                        style="height: auto !important" placeholder="Informations supplémentaires sur la commande...">{{ old('notes') }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="mt-4">
                                    <button type="submit" class="btn btn-primary btn-lg">
                                        <i class="mdi mdi-content-save"></i> Enregistrer la Commande
                                    </button>
                                </div>
                            </form>
                        </div>
...
@section('js')
    ...
    <script>
        ...
        function createNewClient() {
            ...
            const clientPhone = document.getElementById('new_client_phone').value;
            const clientEmail = document.getElementById('new_client_email').value;
            ...
            // Show loading state
            const saveButton = document.querySelector('#newClientModal .btn-primary');
            const originalText = saveButton.innerHTML;
            saveButton.innerHTML = '<i class="mdi mdi-loading mdi-spin"></i> Création en cours...';
            saveButton.disabled = true;
            ...
            const formData = new FormData();
            formData.append('nomDeClient', clientName);
            formData.append('TelephoneDeCLient', clientPhone);
            formData.append('EmailDeClient', clientEmail);
            formData.append('SocieteDeCLient', clientAddress);

            // AJAX call to create client using your existing route
            fetch('{{ route('viewaddClient') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
        ...
        function generateReference() {
            ...
            const reference = `CMD-${year}${month}${day}-${random}`;
